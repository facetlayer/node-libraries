...
export class LexContext {
    ...
    getTokenText(token: Token) {
        return this.str.slice(token.textStart, token.textEnd);
    }
    ...
    consume(match: TokenDef, len: number) {
        ...
        if (match.bracketSide === 'right') {
            ...
            if (match.name !== lookingFor) {
                // Closing bracket doesn't match opening bracket
                result.error = `Bracket mismatch: expected '${lookingFor}' but found '${match.name}'`;
            } else {
