...
import { getStateDatabase, getStateDirectory, getOrCreateStateDirectory } from '../index'
...
describe('user-state-db', () => {
  ...
  describe('getOrCreateStateDirectory', () => {
    it('should create directory if it does not exist', () => {
      const testDir = path.join(TEST_TEMP_DIR, 'create-test')
      process.env.CREATE_TEST_STATE_DIR = testDir

      expect(fs.existsSync(testDir)).toBe(false)
      const result = getOrCreateStateDirectory('create-test')
      expect(result).toBe(testDir)
      expect(fs.existsSync(testDir)).toBe(true)
    })
  })

  describe('getStateDatabase', () => {
    ...
    it('should create db.sqlite file in the state directory', () => {
    ...
    it('should initialize database with provided schema', () => {
    ...
    it('should handle safe-upgrades migration behavior', () => {
      const testDir = path.join(TEST_TEMP_DIR, 'test-app-5')
      process.env.TEST_APP_5_STATE_DIR = testDir
    ...
    it('should work with XDG_STATE_HOME environment variable', () => {
      const xdgBase = path.join(TEST_TEMP_DIR, 'xdg-state')
      process.env.XDG_STATE_HOME = xdgBase
      delete process.env.TEST_APP_6_STATE_DIR
      ...
      const expectedDbPath = path.join(xdgBase, 'test-app-6', 'db.sqlite')
      expect(fs.existsSync(expectedDbPath)).toBe(true)
    })

    it('should return accessible database on multiple calls', () => {
    ...
    it('should handle complex schema with multiple tables and indexes', () => {
      const testDir = path.join(TEST_TEMP_DIR, 'test-app-8')
      process.env.TEST_APP_8_STATE_DIR = testDir
