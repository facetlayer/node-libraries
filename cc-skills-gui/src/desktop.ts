/**
 * Electron entry point for Claude Code Skills Editor.
 *
...
async function main() {
    ...
    console.log(`[cc-skills-gui] Express bound to ${devServerUrl}`);

    await desktopLaunch({
        app,
        appName: 'Claude Code Skills Editor',
        title: 'Claude Code Skills Editor',
        iconPath,
        devServerUrl,
        windowWidth: 1400,
        windowHeight: 900,
...
main().catch((err) => {
    console.error('[cc-skills-gui] Failed to launch desktop app:', err);
    process.exit(1);
});
