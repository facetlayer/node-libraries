 ...
 * 2. Endpoints with Zod schemas that are not compatible with OpenAPI
 */
...
const validApiPrefixEndpoint = createEndpoint({
  method: 'GET',
  path: '/api/status',
  description: 'Endpoint with /api prefix (now allowed)',
  responseSchema: z.object({
    status: z.string(),
  }),
  handler: async () => {
...
// ============================================
// ERROR CASE: Zod schemas incompatible with OpenAPI
...
expressApp.listen(PORT, () => {
  ...
  console.log('  GET  http://localhost:3099/api/status');

  console.log('\nERROR CASE - Bad Zod schemas (may fail):');
